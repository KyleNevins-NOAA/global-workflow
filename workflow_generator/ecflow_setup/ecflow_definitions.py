    ...
    def add_suite_triggers(self, node, triggers, suite_array, parents,
        ...
        for item in working_node.get_full_name_items(index):
            if working_node.get_type() == "family":
                ...
                node_name = f"{item}"
            working_node_index = working_node.get_full_name_items(index).index(item)
            for trigger_item in triggers:
                if working_node.is_list or working_node.is_range:
                    ...
                    trigger_node = ecfTriggerNode(trigger_item, parent_node)
                for trigger in trigger_node.get_full_name_items(working_node_index):
                    trigger_index = trigger_node.get_full_name_items(
                                    working_node_index).index(
                                    trigger)
                    ...
                    if trigger_node.has_operand():
                        operand = bool(trigger_node.get_operand())
                    if trigger_node.trigger_type == "family":
                        trigger_name = f"{trigger.replace('/', '>')}"
                    else:
                        trigger_name = trigger
                    if trigger_node.has_state():
                        if isinstance(trigger_node.get_state(), list):
                            for state in trigger_node.get_state():
                            ...
                            self.add_trigger(trigger_name, node_name,
                                             ...
                                             suite_array=suite_array,
                                             operand=operand)
                    elif trigger_node.has_event():
                        trigger_events_node = trigger_node.get_event()
                        if trigger_node.is_list or trigger_node.is_range:
                            event_index = trigger_index
                        else:
                            event_index = working_node_index
                        for trigger_event in trigger_events_node.get_full_name_items(event_index):
                            self.add_trigger(trigger_name, node_name,
                                             event=trigger_event,
                                             suite=suite,
                                             suite_array=suite_array,
